 ...
 * @param param
  	{
  		url:'接口地址',
  		data:{},//数据
  		succFunc:function(){},//成功回调方法
  		errFunc:function(){}//失败回调方法
  	}
	...
	$.ajax({
		...
		success:function(res){
			if(parseInt(res.status) == 0){//成功
				if(param.succFunc && typeof(param.succFunc) == 'function'){
					param.succFunc(res);
				}else{
					console.log(res)
				}
			}else{//失败
				if(param.errFunc && typeof(param.errFunc) == 'function'){
					param.errFunc(res);
				}else{
					console.log(res.msg)
				}
